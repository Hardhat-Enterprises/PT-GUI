...
def start(window, site, hostName = "localhost", serverPort = 8080):
    try:
        global SITE
        SITE = site
        ...
        window['-OUTPUT-'+sg.WRITE_ONLY_KEY].print("Server stopped.")
        window.refresh()
    ...
    if event == sg.WIN_CLOSED or event == 'Close':
        ...
        if values['-HOST-'] == 'Localhost':
            ...
            if (re.search(r'^\b(?:(?:2(?:[0-4][0-9]|5[0-5])|[0-1]?[0-9]?[0-9])\.){3}(?:(?:2([0-4][0-9]|5[0-5])|[0-1]?[0-9]?[0-9]))\b$', ip_addr) == None):
                ...
                start(window, values['-SITE-'], ip_addr)
