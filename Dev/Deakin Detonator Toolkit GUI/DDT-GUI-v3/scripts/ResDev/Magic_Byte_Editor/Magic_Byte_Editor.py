...
def MAKE_Magic_Byte_Editor():
    ...
    return sg.Window('Magic Byte Tool', layout, size=(1280,720), grab_anywhere=True, finalize=True)

def CHECK_Magic_Byte_Editor(window, event, values):
    Magic_Byte = os.path.dirname(os.path.realpath(__file__)) # Get the current directory the python file is running from
    os.chdir(Magic_Byte) # Set the working Directory to Magic_Byte_Editor
