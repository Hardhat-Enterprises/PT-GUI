...
class PortScan(tk.Frame):
	def __init__(self, parent, controller):
		...
		aboutframe = tk.Label(self, text="    Port Scanner Tool", bg='#3B5262', fg='white', anchor="c", font=abtframefont)
		...
		# Set input box Entry
		label = Label(self,text ="Enter Target IP address:",font=('Calibri', 13), bg='#4D6C84', fg='white', anchor='c').place(rely=0.38, relx=0.16, relheight=0.08, relwidth=0.22)

		self.IP = Entry(self, font=('Calibri', 14))
		self.IP.place(rely=0.38, relx=0.36, relheight=0.08, relwidth=0.3)

		#Set scan button
		Button(self,text="Start Scan",font=('Calibri', 14), bg='#4D6C84', fg='white',anchor='c', command=self.Judgment_IP).place(rely=0.38, relx=0.7, relheight=0.08, relwidth=0.08)
...
def action(ip):
	...
	lock = Lock()
	ports = [21, 22, 23, 25, 53, 69, 80, 135, 445, 443, 137, 139, 143, 8080, 1723, 995, 993, 5900, 68, 520, 514, 162, 49152, 4500, 1521, 1433, 3306, 3389,]
	host = ip
	mt = []
	for p in ports:
