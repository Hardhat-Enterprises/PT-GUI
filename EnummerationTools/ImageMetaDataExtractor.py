...
class IMDExtractor(tk.Frame):
	def __init__(self, parent, controller):
		...
		aboutframe = tk.Label(self, text="    Image MetaData Extractor", bg='#3B5262', fg='white', anchor="c", font=abtframefont)
		...
		# Set input box Entry
		label = Label(self,text ="Please paste the image location",font=('Calibri', 13), bg='#4D6C84', fg='white', anchor='c').place(rely=0.38, relx=0.16, relheight=0.08, relwidth=0.22)

		self.img = Entry(self, font=('Calibri', 13),)
		self.img.place(rely=0.38, relx=0.36, relheight=0.08, relwidth=0.3)

		#Set scan button
		Button(self,text="Extract",font=('Calibri', 13), bg='#4D6C84', fg='white', anchor='c',command=self.myIMDE).place(rely=0.38, relx=0.7, relheight=0.08, relwidth=0.08)
	...
	def myIMDE(self):
		...
		if not imagename:
			label = Label(self,text ="No Image Path Given.",font=('Calibri', 13), anchor='c').place(rely=0.48, relx=0.36, relheight=0.08, relwidth=0.3)
