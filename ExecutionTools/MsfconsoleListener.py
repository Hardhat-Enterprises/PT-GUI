...
class MsfListener(tk.Frame):
    def __init__(self, parent, controller):
        ...
        aboutframe = tk.Label(self, text="    MsfConsole Listener Tool", bg='#3B5262', fg='white', anchor="c",
                              font=abtframefont)
        ...
        labelIP = Label(self, text="Enter Attacker IP Address: ", bg='#4D6C84', fg='white', font=('Calibri', 13),
                        anchor='c')
        ...
        yesButtonIP = Button(self, text="YES", font=('Calibri', 14), bg='#4D6C84', fg='white', anchor='c',
                             command=lambda: ip_retrieved())
        noButtonIP = Button(self, text="NO", font=('Calibri', 14), bg='#4D6C84', fg='white', anchor='c',
                            command=lambda: ip_manual())
        proceedButtonIP = Button(self, text="PROCEED", font=('Calibri', 14), bg='#4D6C84', fg='white', anchor='c',
        ...
        yesButtonPort = Button(self, text="YES", font=('Calibri', 14), bg='#4D6C84', fg='white', anchor='c',
                               command=lambda: port_default())
        noButtonPort = Button(self, text="NO", font=('Calibri', 14), bg='#4D6C84', fg='white', anchor='c',
                              command=lambda: port_manual())
        proceedButtonPort = Button(self, text="PROCEED", font=('Calibri', 14), bg='#4D6C84', fg='white', anchor='c',
                                   command=lambda: [remove_port_entry_boxes(), start_listener()])

        cmdResponse = Text(self, font=('Calibri', 12))
        cmdEntryLabel = Label(self, text="Enter Command (type exit to stop):", bg='#4D6C84', fg='white',
                              font=('Calibri', 13), anchor='c')
        cmdEntryBox = Entry(self, font=('Calibri', 14))
        cmdButton = Button(self, text="Enter", font=('Calibri', 14), bg='#4D6C84', fg='white', anchor='c',
                           command=lambda: command_transmission())

        waitExeLabel = Label(self,
                             text="Listener Configuration Successful!\n(Program will await connection from\nvictim after clicking 'Start Listening' button)",
                             bg='#4D6C84', fg='white', font=('Calibri', 13), anchor='c')
        conSuccessLabel = Label(self, text="Connection Successful!", bg='#4D6C84', fg='white', font=('Calibri', 13),
                                anchor='c')

        endButton = Button(self, text="Terminate", font=('Calibri', 14), bg='#4D6C84', fg='white', anchor='c',
                           command=lambda: [kill_PID(), remove_response_boxes()])

        beginButton = Button(self, text="Launch Tool", font=('Calibri', 18, 'bold'), bg='#4D6C84', fg='white',
                             anchor='c', command=lambda: set_IP())
        beginButton.place(rely=0.54, relx=0.44, relheight=0.08, relwidth=0.12)

        listenButton = Button(self, text="Start Listening", font=('Calibri', 14), bg='#4D6C84', fg='white', anchor='c',
                              command=lambda: session_interact())
        shellButton = Button(self, text="Shell", font=('Calibri', 14), bg='#4D6C84', fg='white', anchor='c',
