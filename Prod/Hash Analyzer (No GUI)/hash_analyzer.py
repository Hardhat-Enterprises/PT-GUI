...
if __name__ == "__main__":
    ...
    while True: # Loop until CTRL+C
        try:
            ...
            choice = input("Pick a number (1-2): ") # Get user input (single hash or file name)

            if (choice == "1"): # If 1, get user input for a hash then check and print the output string
                hash = input("\nPlease enter a hash value:\n")
                print(SingleHash(hash))
            elif (choice == "2"): # If 2, get user input for a file name then check and print the output string
                filename = input("\nPlease enter a file name:\n")
                print(ReadFile(filename))
            else: # If not 1 or 2, print an error message
                print("There is no choice " + choice + ".")
        except KeyboardInterrupt: # If ctrl+c is pressed, quit the hash analyzer
