...
class SshBrute(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Set input box Entry
        label = Label(self, text="Please Enter the Target Address: ", font=('Calibri', 13), bg='#4D6C84', fg='white',
                      anchor='c').place(rely=0.28, relx=0.16, relheight=0.06, relwidth=0.22)
        # Set input box Entry
        label = Label(self, text="Please Enter The Target Username: ", font=('Calibri', 13), bg='#4D6C84', fg='white',
                      anchor='c').place(rely=0.36, relx=0.16, relheight=0.06, relwidth=0.22)
        # Set input box Entry
        label = Label(self, text="Please Enter the File Path: ", font=('Calibri', 13), bg='#4D6C84', fg='white',
                      anchor='c').place(rely=0.44, relx=0.16, relheight=0.06, relwidth=0.22)
        ...
        # Set scan button
        Button(self, text="Start Bruteforce", font=('Calibri', 13), bg='#4D6C84', fg='white', anchor='c',
               command=self.mainsshbrute).place(rely=0.36, relx=0.7, relheight=0.08, relwidth=0.08)
    ...
    def mainsshbrute(self):
        ...
        for i in input_file.readlines():
            ...
            try:
                ...
                if response == 0:
                    success = Label(self,
                                    text=("%s[*] User: %s [*] Password Found: %s%s" % (line, username, password, line)),
                                    font=('Calibri', 13))
                    ...
                    result = Label(self,
                                   text=("[*] User: %s [*] Password: %s => Login Incorrect <=" % (username, password)),
                                   font=('Calibri', 13))
                    result.place(rely=0.68, relx=0.36, relwidth=0.3)
                    print("[*] User: %s [*] Password: %s => Login Incorrect <=" % (username, password))
                elif response == 2:
                    result = Label(self, text=("[*] Connection Could Not Be Established To Address: %s" % (host)),
                                   font=('Calibri', 13))
                    result.place(rely=0.68, relx=0.36, relwidth=0.3)
