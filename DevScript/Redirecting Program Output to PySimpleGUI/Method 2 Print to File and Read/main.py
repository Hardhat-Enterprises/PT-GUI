...
from hash_analyzer import hash_analyze
import os
...
layout =   [[sg.Text('Hash Analyzer Print To File')],
...
window = sg.Window('Hash Analyzer Print To File', layout)
...
while True:
    ...
    if event.startswith('-ENTER-'):
        inp = values['-IN1-']
        combo = values['-COMBO-']
        hash_analyze(inp, combo)
        try:
            output_file = open(os.path.abspath(os.path.dirname(__file__)) + "/" + "_temp", "r")
            for line in output_file:
                line = line.strip("\n")
                window['-OUTPUT-'+sg.WRITE_ONLY_KEY].print(line)
                window.refresh()
