...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Check a given hash for its possible hashing algorithm.')
    group = parser.add_mutually_exclusive_group()
    group.add_argument('-hs', '--hash', help='input a single hash')
    group.add_argument('-f', '--file', help='input a file name')
    if len(sys.argv)==1:
    ...
    if(args.hash != None):
        hash_analyze(args.hash, 'Hash')
    elif(args.file != None):
        hash_analyze(args.file, 'File')